...
bool shouldDropPacket1(double lossRate) {
    double randomValue = (double)rand() / RAND_MAX;
    if (randomValue < lossRate) {
        BaseConnectionManager::incrementDroppedPackets();  // 增加丢包计数
        return true;
    }
    return false;
}
void ChannelAccess::sendToChannel(cPacket* msg)
{
    ...
    // 获取丢包率参数，这里假设您已经在NEC文件中定义了packetLossRate参数
    // 并且已经在初始化阶段读取了这个参数
    double packetLossRate = BaseConnectionManager::lossRate;

    // 根据丢包率决定是否发送数据包
//    if (shouldDropPacket1(packetLossRate)) {
//        EV << "C2R Packet dropped due to loss rate: " << packetLossRate << endl;
...
//    }
        ...
        if (useSendDirect) {
            if (gate->isVector()) {
                for (int gateIndex = gate->getBaseId(); gateIndex < gate->getBaseId() + gate->size(); gateIndex++) {
                    if (shouldDropPacket1(packetLossRate)) {
                        EV << "C2C Packet dropped due to loss rate: " << packetLossRate << endl;
    ...
    EV << "The lost packages count" << BaseConnectionManager::droppedPackets << endl;
    delete msg;
}
