...
double BaseConnectionManager::lossRate = 0.0;
int BaseConnectionManager::droppedPackets = 0;  // 静态成员变量定义和初始化
namespace {
/**
...
void BaseConnectionManager::initialize(int stage)
{
    ...
    if (stage == 0) {
        double packetLossRate = par("lossRate").doubleValue();
        setLossRate(packetLossRate);
...
bool BaseConnectionManager::unregisterNic(cModule* nicModule)
{
    ...
    // get all affected grid squaresF
    CoordSet gridUnion(74);
    GridCoord cell = getCellForCoordinate(nicEntry->pos);
    if ((gridDim.x == 1) && (gridDim.y == 1) && (gridDim.z == 1)) {
        gridUnion.add(cell);
...
bool BaseConnectionManager::shouldDropPacket(double lossRate) {
    double randomValue = (double)rand() / RAND_MAX;
    return randomValue < lossRate;
}


void BaseConnectionManager::setLossRate(double newLossRate) {
