...
public:
    static bool shouldDropPacket(double lossRate);  // 静态成员函数声明
    static double lossRate;//定义丢弃率
    static void setLossRate(double newLossRate);
    ~BaseConnectionManager() override;
    /** @brief Needs two initialization stages.*/
    static int droppedPackets;  // 用于统计丢失的数据包数量
